<html>
	...
	<body bgcolor="skyblue">
		...
		<?php 
			...
			$query="update performance set test2='$ans' where handle='$hand' and test2='-1'";
			mysql_query($query,$conn);
			$query="select * from performance where handle='$hand'";
			$result=mysql_query($query);
			...
			$avg=$sum/$x;
			$query="update performance set average='$avg' where handle='$hand'";
			mysql_query($query,$conn);
			if($avg>=4)$rank="GrandMaster";
			else if($avg>=3)$rank="Master";
			else if($avg>=2)$rank="Specialist";
			else $rank="Newbie";
			$query="update performance set ranking='$rank' where handle='$hand'";
			mysql_query($query,$conn);
